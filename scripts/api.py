...
class API:
    def __init__(self, server_auth: str) -> None:
        self.auth: dict[str, str] = json.loads(server_auth)
    ...
    def level_leaderboard(self, identifier: str) -> list[Placement] | None:
        ...
        response: Response | None = safe_get(url, self.auth)
    ...
    def level_list(self, list_type: str) -> list[Level] | None:
        url: str = (
            f"{SERVER_API}list?max_format_version={FORMAT_VERSION}&type={list_type}"
        )
    ...
    def full_level_list(self, list_type: str) -> list[Level] | None:
        ...
        while True:
            url: str = (
                f"{SERVER_API}list?max_format_version={FORMAT_VERSION}&type={list_type}&page_timestamp={page_timestamp}"
            )
